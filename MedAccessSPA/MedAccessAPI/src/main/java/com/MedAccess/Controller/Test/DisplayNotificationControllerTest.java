...
@PrepareForTest({DisplayNotificationControllerTest.class,DisplayNotificationController.class,DisplayNotificationService.class})
public class DisplayNotificationControllerTest {
	
	
	@Spy
	private DisplayNotificationService mockDisplayNotificationService;
	...
	public void testGetDoctorAppointment(){
		...
		Mockito.doReturn(mockDoctorNotificationList).when(mockDisplayNotificationService).getDoctorAppointment(Mockito.anyString());

		
		//Method call
		ResponseEntity<List<DoctorNotification>> response = displayNotificationController.getDoctorAppointment("1");
		
		List<DoctorNotification> resultDoctorNotification = response.getBody();
